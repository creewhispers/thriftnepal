...
export const Route = createFileRoute("/login")({
  ...
  head: () => ({
    meta: [
      { title: "Sign in — thriftdepo.np" },
      { name: "description", content: "Sign in or create an account to buy and sell on thriftdepo.np." },
    ],
  }),
  component: LoginPage,
});
    ...
    <div className="min-h-screen grid place-items-center bg-background px-4">
      <div className="w-full max-w-md">
        <Link to="/" className="flex items-center gap-2 justify-center mb-8">
          ...
          <span className="font-display text-xl tracking-tight">thriftdepo<span className="text-accent">.</span>np</span>
        </Link>

        <div className="rounded-3xl border border-border bg-card p-8">
          <h1 className="font-display text-3xl">{mode === "signin" ? "Welcome back" : "Join the depot"}</h1>
          <p className="text-sm text-muted-foreground mt-1">
            {mode === "signin" ? "Sign in to buy, sell, and save." : "Create an account in seconds."}
          </p>

          <button
            onClick={handleGoogle}
            disabled={busy}
            className="mt-6 w-full inline-flex items-center justify-center gap-2 rounded-full border border-border bg-background py-2.5 text-sm font-medium hover:bg-secondary transition disabled:opacity-50"
          >
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            />
            <input
              type="password" placeholder="Password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)}
              className="w-full rounded-xl border border-border bg-background px-4 py-2.5 text-sm outline-none focus:border-accent transition"
            />
